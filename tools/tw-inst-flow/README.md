...
- **櫃買網站的 SSL 憑證鏈不完整**（實測 `Verify return code: 21`，而且同一台主機不同連線結果不一致）。程式會重試 6 次；若你的環境一直失敗，可以設 `TPEX_INSECURE_SSL=1` 略過驗證再跑——抓的是公開行情資料、不涉及帳密，但這代表無法確認連到的是不是真的櫃買，要不要開自己決定。不開的話就是報告少了上櫃那半邊，其他照常。
