...
| `TPEX_INSECURE_SSL` | 未設 | 設成 `1` 會略過櫃買網站的憑證驗證，見下方說明 |
...
- 只統計 **4 碼且不以 0 開頭的普通股**，ETF（0050、0056、00878…）、權證、興櫃都被濾掉了。
- **上櫃資料只有最新一個交易日。** 櫃買中心的開放資料 API 不提供歷史查詢，所以「連續買超天數」只算得出上市股票；指定 `--date` 查過去日期時，報告只會有上市。
- **櫃買網站有兩個實測到的毛病，程式都已自動處理，不需要你做任何事：**
  1. 它的伺服器只送葉憑證、沒送中介憑證（實測 `Verify return code: 21`），Linux 上的 python 和 curl 都會驗證失敗。程式會照憑證裡的 AIA 位址自己把中介憑證抓回來補上再連一次（Windows / macOS 本來就會自動做這件事）。**根憑證照常驗證，不是關掉驗證。**
  2. 它的 800KB 資料檔常常傳到一半被切斷。程式會先試整包（gzip），失敗就改用 HTTP Range 每次 256KB 分段抓，斷掉只重抓那一段。
  萬一哪天還是連不上，可以設 `TPEX_INSECURE_SSL=1` 略過憑證驗證——抓的是公開行情、不涉及帳密，但這代表無法確認連到的是不是真的櫃買，要不要開自己決定。不開的話就是報告少了上櫃那半邊，其他照常。
